...
public class ProposeRecipeCandidatesTool implements AgentTool {
    ...
    // Stand-in for a real user_profile (PRD step 9, which doesn't exist yet): a generous
    // default kitchen with no known allergies. Every candidate is checked against the same
    // constraints today; once profiles exist, this becomes a per-user lookup instead.
    private static final UserConstraints DEFAULT_CONSTRAINTS = new UserConstraints(
            Set.of(),
    ...
    public String execute(Long userId, JsonValue input) {
        ...
        for (CandidateInput candidateInput : candidates) {
            ...
            RecipeValidationResult validation = recipeValidator.validate(userId, candidate, DEFAULT_CONSTRAINTS);
            if (!validation.valid()) {
